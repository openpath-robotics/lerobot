"""
HDF5 → LeRobot v3.0 변환 스크립트 (openarm_0427)
OpenArm 16 DOF, 카메라 3대 (cam_top / cam_wrist_left / cam_wrist_right)
이미지: 320x240 JPEG bytes → 그대로 사용
"""
...
# 설정
# ============================================================
HDF5_DIR = os.path.expanduser("~/openarm_0427")
OUTPUT_DIR = os.path.expanduser("~/openarm_0427_smolvla")
FPS = 30
ROBOT_TYPE = "openarm"
NUM_WORKERS = 8
...
IMAGE_SHAPE = [240, 320, 3]  # HxWxC (이미 320x240)
# ============================================================


def pass_jpeg(raw) -> dict:
...
def process_single_episode(args):
    ...
    for frame_idx in range(n_frames):
        ...
        for hdf5_key, lerobot_key in zip(cam_hdf5_keys, cam_lerobot_keys):
            row[lerobot_key] = pass_jpeg(cam_data[hdf5_key][frame_idx])
        rows.append(row)
...
def main():
    print("=" * 60)
    print("HDF5 → LeRobot v3.0 변환 (openarm_0427)")
    print("=" * 60)
