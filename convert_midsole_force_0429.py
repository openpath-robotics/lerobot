"""
HDF5 → LeRobot v3.0 변환 스크립트 (midsole_grasp_rotate_force_0429)
OpenArm 28D state (16D joint+gripper + 12D f_ext), action 16D
카메라 3대 (cam_top / cam_wrist_left / cam_wrist_right)
이미지: raw uint8 640x480 → 320x240 JPEG 인코딩

# ================================================================
# TODO (새 데이터셋으로 복사할 때 반드시 수정할 항목들)
#
# 1. HDF5_DIR   : HDF5 파일이 들어있는 폴더 경로
# 2. OUTPUT_DIR : 변환 결과를 저장할 폴더 경로
# 3. FPS        : 데이터 수집 주파수 (보통 30)
# 4. EPISODE_NUM_RANGE : 포함할 에피소드 번호 범위 (파일명 숫자 기준)
#                        예) 파일이 episode_0.hdf5 ~ episode_49.hdf5 → (0, 49)
# 5. EPISODE_TASK_MAP  : (시작ep, 끝ep, "언어 라벨") 리스트
#                        태스크가 1개면 한 줄, 여러 구간이면 여러 줄
#                        예) [(0,24,"task A"), (25,49,"task B")]
# 6. CAMERA_MAPPING    : HDF5 내 이미지 키 → LeRobot 카메라 키 매핑
#                        - HDF5 키는 h5py로 열어서 list(f["images"].keys()) 로 확인
#                        - 빈 카메라(all-zero)는 제외
#                        - LeRobot 키는 camera1부터 순서대로
# 7. state_names (main 함수 하단) : observation.state 각 차원의 이름 리스트
#                        - state_dim과 길이가 반드시 일치해야 함
#                        - 일반 16D: [f"R_joint_{i}" for i in range(7)] + ["R_gripper"]
#                                  + [f"L_joint_{i}" for i in range(7)] + ["L_gripper"]
#                        - 28D(+force): 위에 R_fext_0..5, L_fext_0..5 추가
# ================================================================
"""
...
# 설정
# ============================================================
HDF5_DIR = "/media/youngwoo/OPR_LINUX/shoes_demo/midsole_grasp_rotate_force_0429"  # TODO(1): HDF5 파일 폴더 경로
OUTPUT_DIR = os.path.expanduser("~/midsole_grasp_rotate_force_0429_smolvla")        # TODO(2): 출력 폴더 경로
FPS = 30                                                                             # TODO(3): 데이터 수집 FPS
ROBOT_TYPE = "openarm"
NUM_WORKERS = 8

EPISODE_NUM_RANGE = (0, 51)  # TODO(4): 포함할 에피소드 번호 범위 (파일명 숫자 기준, 양쪽 포함)

# TODO(5): 태스크 구간 및 언어 라벨 설정
#   형식: (시작ep, 끝ep, "언어 라벨") — 여러 구간이면 줄 추가
#   예) 단일 태스크: [(0, 51, "grasp and rotate midsole")]
#   예) 2개 태스크:  [(0, 24, "grasp midsole"), (25, 51, "rotate midsole")]
EPISODE_TASK_MAP = [
    (0, 51, "grasp and rotate midsole"),
]
...
# TODO(6): HDF5 카메라 키 → LeRobot 카메라 키 매핑
#   - HDF5 키 확인: python -c "import h5py; f=h5py.File('episode_0.hdf5','r'); print(list(f['images'].keys()))"
#   - 빈 카메라(all-zero, 평균≈0)는 제외
#   - LeRobot 키는 camera1부터 순서대로 (알파벳 순서로 매핑 권장)
CAMERA_MAPPING = {
...
def process_single_episode(args):
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        q_pos  = f["q_pos"][:].astype(np.float32)   # (T, 28)
        action = f["action"][:].astype(np.float32)   # (T, 16)
...
def main():
    print("=" * 60)
    print("HDF5 → LeRobot v3.0 변환 (midsole_grasp_rotate_force_0429)")
    print("=" * 60)
    ...
    with h5py.File(hdf5_files[0], "r") as f:
        ...
        print(f"State dim: {state_dim} (joint+gripper+f_ext), Action dim: {action_dim}, Frames/ep: {n_frames_first}")
    ...
    stats["frame_index"]   = {"min": [0], "max": [max(ep_lengths) - 1], "mean": [float(all_indices.mean())], "std": [float(all_indices.std())], "count": [total_frames]}
    stats["episode_index"] = {"min": [0], "max": [n_eps - 1], "mean": [float((n_eps - 1) / 2)], "std": [float(np.arange(n_eps).std())], "count": [total_frames]}
    stats["index"]         = {"min": [0], "max": [total_frames - 1], "mean": [float((total_frames - 1) / 2)], "std": [float(all_indices.std())], "count": [total_frames]}
    n_tasks = len(TASK_DESCRIPTIONS)
    stats["task_index"]    = {"min": [0], "max": [n_tasks - 1], "mean": [float((n_tasks - 1) / 2)], "std": [float(np.arange(n_tasks).std()) if n_tasks > 1 else 0.0], "count": [total_frames]}

    with open(f"{OUTPUT_DIR}/meta/stats.json", "w") as f:
        json.dump(stats, f, indent=4)

    # TODO(7): state 각 차원의 이름 리스트 — state_dim과 길이가 반드시 일치해야 함
    #   16D (일반):  [f"R_joint_{i}" for i in range(7)] + ["R_gripper"]
    #              + [f"L_joint_{i}" for i in range(7)] + ["L_gripper"]
    #   28D (+force): 아래처럼 R_fext, L_fext 6개씩 추가
    state_names = (
        [f"R_joint_{i}" for i in range(7)] + ["R_gripper"] + [f"R_fext_{i}" for i in range(6)] +
        [f"L_joint_{i}" for i in range(7)] + ["L_gripper"] + [f"L_fext_{i}" for i in range(6)]
    )
    ...
    info = {
        ...
        "video_path": None,
        "features": {
            **{k: {"dtype": "image", "shape": IMAGE_SHAPE, "names": ["height", "width", "channel"], "fps": FPS}
               for k in CAMERA_MAPPING.values()},
            "observation.state": {"dtype": "float32", "shape": [state_dim], "names": state_names, "fps": FPS},
            "action":            {"dtype": "float32", "shape": [action_dim], "names": [f"action_{i}" for i in range(action_dim)], "fps": FPS},
    ...
    print(f"State dim: {state_dim} (7+1+6) × 2팔")
